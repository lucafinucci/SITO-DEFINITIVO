...
if ($action === 'toggle') {
    ...
    if (!$pacchettoId || !in_array((int)$attivo, [0, 1], true)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Dati non validi']);
        exit;
    }
    $stmt = $pdo->prepare('UPDATE pacchetti SET attivo = :attivo WHERE id = :id');
    $stmt->execute(['attivo' => (int)$attivo, 'id' => $pacchettoId]);
...
$stmt = $pdo->prepare('
    INSERT INTO pacchetti (nome, descrizione, prezzo_mensile, attivo)
